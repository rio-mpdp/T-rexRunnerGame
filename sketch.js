...
function setup() {
  ...
  trex = createSprite(25, height-20, 20, 20);
  trex.addAnimation("runs", trex_runs);
  trex.addAnimation("dies", treximage);
  trex.scale = 0.5
  trex.setCollider("rectangle", 0, 0, 100, 100)
  edges = createEdgeSprites();
  ground = createSprite(width/2,height-10, width, 15)
  ground.addImage(groundimage);
  invisableground = createSprite(width/2,height, width, 10);
...
function createraptors() {
  if (frameCount % 150 == 0) {
    r = createSprite(width, Math.round(random(10, height-100)), 10, 10);
    r.velocityX = -2;
...
function obstacle1() {
  if (frameCount % 100 == 0) {
    obstacle = createSprite(width, height-20, 10, 10);
...
function draw() {
  ...
  if (gamestate == 0) {
    ...
    if ((keyDown("space") ||touches.length>0 )&& trex.y >= height-31) {
      jump.play()
      trex.velocityY = -10;
